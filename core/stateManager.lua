...
function GameStateManager:addStateToQueue(state, priority, tbl, bufferData)
    table.insert(GameStateManager.stateQueue, {state = state, priority = priority, args = tbl})
    sortQueue()
end
...
function GameStateManager:processStateQueue()
    if not self.currentState and #self.stateQueue > 0 then
        ...
        self:setState(nextState.state, nextState.args)
    end
    ...
    if self.currentState then
        ...
        if finish then
            if #self.stateQueue > 0 then
                local nextState = self.stateQueue[1]
                table.remove(self.stateQueue, 1)
                return self:setState(nextState.state, nextState.args)
            end
        end
    end
end
...
function GameStateManager:setState(newState, tbl)
